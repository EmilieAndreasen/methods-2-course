...
```
We see that there is a minimum around $x \approx 0$.
...
x = -10
repeat{
  print(paste("for x = ", x," the closest minimum is at x =",optim(x, m1, method = "CG")$par))
  x = x +1
  if (x > 10) break("X limit reached")
}
```
...
```{r}
#if negative = maximum, has to be positive :)
```
