...
```
Due to the uncertainty regarding our beta estimates we can have drip ranging from -40 to 60 when swag = 2, depending on the coefficients. We can of course compute confidence intervals, mean, median, SD etc. for this distribution.
...
A linear regression is fit on high school students modeling grade point average given household income. Write R code to compute the 90% predictive interval for the difference in grade point average comparing two students, one with household incomes of
...
m = stan_glm(gpa ~ earning, data = lul)
summary(m)
...
```{r}
df_kid <- read_csv("C:/Users/Emili/Documents/University/2nd semester/methods-2-course/Data for the book/KidIQ/data/kidiq.csv")
df_kid

bayes_model <- stan_glm(kid_score ~mom_iq, data = df_kid)
summary(bayes_model)
```

```{r}
samples1 <- as.matrix(bayes_model)
