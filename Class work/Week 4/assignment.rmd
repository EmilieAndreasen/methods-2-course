...
```
```{r}
#Estimate the uncertainty surrounding the point estimate difference.
sqrt(1.5^2+2^2)
```
...
sample(c("friday","saturday","sunday"), size = 10, replace = TRUE, prob = c(0.275,0.225,0.5))
...
It prints "please enter a positive integer"
...
```{r}
fun_function <- function(string1, string2){
  if(nchar(string1)>2){
    paste("The lengths is",nchar(string1)+nchar(string2))
  }else{
    "Fuck you"
  }
...
```{r}
# 5.1
scoring_func <- function(probability){
  ...
  while(last||current){
    last=current
    current = sample(c(T, F), size = 1, replace = TRUE, prob = c(probability, 1-probability))
    if (current) {
      k = k+1
    }
    n = n+1
...
res = scoring_func(0.6)
ns = c(1:1000) #number of trials
ks = c(1:1000) #number of successes
for (i in 1:1000) {
  res = scoring_func(0.6)
  ns[i] = res$n
  ks[i] = res$k
...
simulation = function(){
  shots = sample(c(1,0), size = 10, prob = c(0.4, 0.6), replace = T)
  return(sum(shots))
}
...
z = c(1:1000)
for (i in 1:1000){
  x = mean(rnorm(n= 100, mean=69, sd=2.9))
  y = mean(rnorm(n= 100, mean=63.7, sd=2.7))
  z[i]=x-y
}
hist(z)
sd(z)
...
```{r}
# 5.8
x = rnorm(1000, mean=0.1, sd=0.17)
diff = abs(x-0.1) < 2*0.17
diff
sum(diff)/10
```
