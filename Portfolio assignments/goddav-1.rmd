...
```{r}
pacman::p_load(rstan, rstanarm, tidyverse, ggplot2, car, dplyr)
```
...
hist(simulations$mom_age)
hist(simulations$sigma)
summary(model)
str(simulations)
```
...
```{r}
train_data = kid_df %>% slice_head(n=200)
test_data = kid_df %>% slice_tail(n=200)

draw = 2

model_4 = stan_glm(kid_score ~ mom_age * mom_hs, data=train_data, refresh=0)
...
test_data = test_data %>%
  ...
  relocate(kid_score, .after=age_factor) %>% 
  ...
  mutate(label=if_else(label=="kid_score", "real", "predicted"))
...
mom_no_hs_df %>% ggplot(aes(x = age_factor, y = kid_score, col=label)) +
  geom_point(position= position_jitter(width = 0.1, height = 0)) +
  geom_abline(intercept = intercept, slope = slope.mom_age) +
  ggtitle("Mother does not have high school education")

mom_hs_df %>% ggplot(aes(x = age_factor, y=kid_score, col=label)) +
  geom_point(position = position_jitter(width = 0.1, height = 0)) +
  geom_abline(
    intercept = intercept + slope.mom_high_school,
    slope = slope.mom_age + slope.interaction
...
```
In conclusion, our Intercept coefficient tells us that when beauty scores are equal to 0, evaluations would be 4.0. For each increment in beauty score, your evaluation would increase by 0.1. The estimate of these values have quite peak distibutions with a standard deviation of 0.0. The distribution of residuals are quite wide, as sigma = 0.5, which describes the resudual standard deviation.
...
```{r}
new1 <- data_frame(age=50, beauty=-1, female=1)
new2 <- data_frame(age=60, beauty=-0.5, female=0)
y_postpred1 <- posterior_predict(model_bea3, newdata = new1,draws=1000 )
y_postpred2 <- posterior_predict(model_bea3, newdata = new2,draws=1000 )

y_diff <- (y_postpred2-y_postpred1)
y_diff <- data.frame(y_diff)
result <- filter(y_diff, X1>0)

nrow(result)

y_diff %>% 
  filter(rowSums(across(where(is.numeric)) > 0)1000)
...
plot(density(y_postpred1-y_postpred2))
length(filter(y_diff>0))
y_diff
